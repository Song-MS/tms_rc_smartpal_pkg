...
#include "moveit_msgs/srv/get_position_fk__rosidl_typesupport_fastrtps_c.h"
...
#include "rosidl_generator_c/string.h"  // fk_link_names
#include "rosidl_generator_c/string_functions.h"  // fk_link_names
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: fk_link_names
  {
    ...
    if (ros_message->fk_link_names.data) {
      rosidl_generator_c__String__Sequence__fini(&ros_message->fk_link_names);
    }
    if (!rosidl_generator_c__String__Sequence__init(&ros_message->fk_link_names, size)) {
      return "failed to create array for field 'fk_link_names'";
    }
    auto array_ptr = ros_message->fk_link_names.data;
    for (size_t i = 0; i < size; ++i) {
      std::string tmp;
...
#include "geometry_msgs/msg/pose_stamped__functions.h"  // pose_stamped
  ...
  void * untyped_ros_message)
{
  ...
  // Field name: pose_stamped
  {
    ...
    if (!geometry_msgs__msg__PoseStamped__Sequence__init(&ros_message->pose_stamped, size)) {
      return "failed to create array for field 'pose_stamped'";
    }
    auto array_ptr = ros_message->pose_stamped.data;
    for (size_t i = 0; i < size; ++i) {
      if (!callbacks->cdr_deserialize(
